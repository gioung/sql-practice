...
select min(salary) as '최고연봉',max(salary) as '최저연봉' from salaries;

-- 문제6.최고 어린 사원의 나이와 최 연장자의 나이는?
select date_format(CURDATE(),'%Y')-date_format(min(birth_date),'%Y')+1 as 'older',date_format(CURDATE(),'%Y')-date_format(max(birth_date),'%Y')+1 as 'younger' from employees;